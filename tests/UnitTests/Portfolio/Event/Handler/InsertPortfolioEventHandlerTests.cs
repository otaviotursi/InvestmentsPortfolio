...
namespace UnitTests.Portfolio.Event.Handler
{
    ...
    public class InsertPortfolioEventHandlerTests
    {
        private readonly Mock<IKafkaProducerService> _kafkaProducerServiceMock;
        private readonly InsertPortfolioEventHandler _handler;

        public InsertPortfolioEventHandlerTests()
        {
            _kafkaProducerServiceMock = new Mock<IKafkaProducerService>();
            _handler = new InsertPortfolioEventHandler(_kafkaProducerServiceMock.Object);
        }
        ...
        {
            // Arrange
            var portfolioEvent = new InsertPortfolioEvent
            {
                CustomerId = 1,
